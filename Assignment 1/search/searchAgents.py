...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # state = (position, (corners to be visited))
        # goal state means all corners were visited
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            # given code to check if the new position is a wall
            x, y = state[0]
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            ...
            if not hitsWall: # if new position is not a wall:
                nextPos = (nextx, nexty)
                cornerList = list(state[1]) # more flexible than a set

                if nextPos in cornerList: # visiting a corner
                    cornerList.remove(nextPos) # the goal is to remove all corners eventually

                nextState = (nextPos, cornerList)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    if not state[1]: # all corners were visited, h = 0
        return 0
